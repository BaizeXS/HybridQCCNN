...
def test_initialization(default_kernel):
    ...
    assert default_kernel.kernel_size == (TEST_PARAMS['kernel_size'], TEST_PARAMS['kernel_size'])
...
@pytest.mark.parametrize("invalid_param", [
    {'quantum_channels': 0, 'error_match': "quantum_channels must be a positive integer"},
    {'kernel_size': 0, 'error_match': "kernel_size must be a positive integer"},
    {'num_param_blocks': 0, 'error_match': "num_param_blocks must be a positive integer"},
])
...
def test_input_dimension_validation(default_kernel, sample_weights):
    ...
    with pytest.raises(ValueError, match=f"Expected {default_kernel.num_qubits} inputs"):
