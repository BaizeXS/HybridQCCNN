...
from components import AggregationMethod, OutputMode, Quanv2d
...
def test_quanv2d_initialization(default_layer):
    ...
    assert default_layer.kernel_size == (
...
def test_quanv2d_input_validation(default_layer):
    ...
    with pytest.raises(
        ValueError, match=f"Expected {TEST_PARAMS['in_channels']} channels"
    ):
        default_layer(torch.randn(1, 3, 14, 14))


@pytest.mark.parametrize(
    "output_mode,preserve_info",
    [
