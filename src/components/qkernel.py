...
class QKernel:
    ...
    def _validate_params(
        ...
        if kernel_circuit and not weight_shapes:
            raise ValueError("Must provide weight_shapes when using custom kernel circuit")

    def _default_circuit(self, inputs: ArrayLike, weights: ArrayLike) -> list[ExpectationMP]:
        ...
        if weights.shape != (self.num_param_blocks, 2 * self.num_qubits):
            raise ValueError(
                f"Incorrect weights shape. Expected {(self.num_param_blocks, 2 * self.num_qubits)}, got {weights.shape}")
        ...
        for layer in range(self.num_param_blocks):
            # Entanglement
            for qubit in range(self.num_qubits):
                qml.CRZ(weights[layer, qubit],
                        wires=[qubit, (qubit + 1) % self.num_qubits])
            # Rotation
            for qubit in range(self.num_qubits):
    ...
    def __repr__(self) -> str:
        ...
        return (f"QKernel(num_qubits={self.num_qubits}, "
