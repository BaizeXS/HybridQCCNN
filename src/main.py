...
def parse_args():
    ...
    parser.add_argument(
        "--model_name", type=str, default="default", help="模型实例名称"
    )
...
def prepare_data(config, data_dir):
    ...
    # 加载测试数据集
    test_dataset = get_dataset(config.data, data_dir, test_transform)
...
def main():
    ...
    logger.info("开始训练...")
    try:
        model_manager.train(train_loader, val_loader, use_amp=args.amp)
